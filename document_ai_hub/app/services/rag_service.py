...
class RAGService:
    ...
    def __init__(self, client: Groq, model_name: str, vector_service: VectorService):
        self.client = client
    ...
    def build_context(docs) -> str:
        ...
        for i, doc in enumerate(docs, start=1):
            block = f"[Document {i} | Source: {doc.metadata.get('source','unknown')}]\n{doc.page_content}"
            context_blocks.append(block)
...
"""
        ...
        if mode == "Legal_mode":
            ...
            prompt_mode = "Act as a Clinical Data Specialist. Extract patient history with a strict distinction between: Chief Complaint: (The reason the patient sought care).Presenting Symptoms: (What they felt). Etiology/Clinical Cause: (The underlying reason identified by the physician, e.g., dietary indiscretion, medication non-adherence).Treatment Plan: (Specific dosage changes and follow-ups). Ensure you do not confuse physical activities (e.g., mowing the lawn) with clinical causes (e.g., sodium intake/CHF). Use the 'Source' tags provided in the text for every data point."
...
"""
    ...
    def generate_answer(
        ...
        docs = self.vector_service.similarity_search(query, k=3, file_domain=doc_domain)

        context = self.build_context(docs)

        prompt = self.build_prompt(query, context, response_format, mode)
